...
def get_team_info(url = None):
    all_data          = []
    table_titles_list = []
    table_datas_list  = []
    if url != None:
        team_info_web_obj = requests.get(url)
...
def matchup_and_host_info(url = 'https://pleagueofficial.com/schedule-regular-season'):
...
def match資訊(team_dataframe, match_up_dataframe, main_team):
    specific_match_up_dataframe       = match_up_dataframe[(match_up_dataframe['主隊'] == main_team) | (match_up_dataframe['客隊'] == main_team)]
